<template>
  <v-navigation-drawer
    ...
    <v-list>
      ...
      <v-list-tile @click="changeRoute('Domain', 2)">
        <v-list-tile-action>
          <v-icon>folder_open</v-icon>
        </v-list-tile-action>
        <v-list-tile-title :class="[{'active': selectedIndex === 2}, 'item-title' ]"> {{ $t('Domain') }}</v-list-tile-title>
      </v-list-tile>
...
export default {
  ...
  methods: {
    changeRoute(routeName, selectedIndex) {
      ...
      var user = localStorage.getItem("manager_user");
      console.log(user);
      if (!user) {
